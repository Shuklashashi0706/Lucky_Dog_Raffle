...
export async function getSelectedWalletHtml(
  selectedWallet,
  prefixWithMessage = "",
  selectedChainObjKey = ""
) {
...
export function replyWithHTMLAndInlineKeyboard(
  ctx,
  htmlMessage,
  inlineKeyboard
) {
...
export async function playCommand(ctx, wallets) {
  ...
  if (pauseStatus) {
    ...
    const walletsBtns = wallets.map((wallet) =>
      createCallBackBtn(wallet.name, `play-wallet-${wallet.name}`)
    );
...
export async function refundCommand(ctx, wallets) {
  ...
  const walletsBtns = wallets.map((wallet) =>
    createCallBackBtn(wallet.name, `refund-wallet-${wallet.name}`)
  );
  inlineKeyboard.push(walletsBtns, [backToMainMenu]);
...
export async function walletsCommand(ctx, wallets) {
  ...
  if (wallets && wallets.length) {
    try {
      ...
      htmlMessage =
        "<b>⚠️ Failed to retrieve wallets. Please try again later.</b>";
    }
  }
